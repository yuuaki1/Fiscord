...
export default function ServerModal ({open, handleClose}) {
    ...
    const AddServer = async (e) => {
        if (serverName === ''){
            alert('Please enter valid server name')
        }
        else{
            e.preventDefault()
        try{
          await addDoc(collection(db, 'servers'),{serverName : serverName})
          setServerName('')
          handleClose()
        }catch(error){
            console.log(error)
        ...
        <div className="fixed inset-0 bg-[#383c3c] h-[400px] w-[500px] mt-[250px] ml-[600px] shadow-2xl flex justify-center items-center rounded-lg">
            <div className="items-center justify-center">
                ...
                <form className="p-3 pb-5 pt-20 flex flex-col justify-center" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <input type = 'text' required placeholder="Enter your server name" onChange={(e) => setServerName(e.target.value)} className="bg-[#201C24] text-white p-2 rounded-md w-[420px] h-10 mt-2" />
                    <div></div>
                </form>
                <div className="flex items-center justify-between p-5 bg-[#29292e] rounded-lg">
                    ...
                    <button className="bg-[#646cf8] duration-200  hover:bg-[#4d52b5] text-white py-2 px-6 rounded-sm center text-sm font-bold" onClick={AddServer}>Create</button>
                </div>
            </div>
        </div>
    )
