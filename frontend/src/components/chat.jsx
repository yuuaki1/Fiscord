...
export default function Chat () {
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex-grow overflow-y-scroll h-[780px] ">
                {messages?.docs?.map((doc) => {

                    const { message, name, photoURL, email, timestamp } = doc.data()
                    return (
                        <Message key={doc.id} id={doc.id} name={name} photoURL={profile} email={email} message={message} timestamp={timestamp} />
                    )
                })}
